...
class UserSingleStep(torch.nn.Module):
    ...
    def plot(self, user_data, scale=False, print_labels=False,neptune=None,subdir=None):
        ...
        if data.shape[0] == 1:
            #plt.axis("off")
            img = numpy.uint8(data[0].permute(1, 2, 0).expand(-1,-1,3).cpu().numpy()*255)
            img=Image.fromarray(img)
            filename=f"images/img_{subdir}_{time.time()}.jpg"
            img.save(filename)
            if neptune is not None:
                neptune[f"train/images_{subdir}"].log(File(filename))
